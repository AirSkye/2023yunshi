...
class ctl_ffsm_h5_index
{
	...
	public $cache_prefix = 'yshi4.mizheba.net';
	...
	public function __construct()
	{
        ...
        tpl::assign('sh_title',"真人在线算命2023年事业财运详解、塔罗运势占卜分析");
  ...
  public static function get_hdjr($date)
	{
		...
		$url = "http://zhouxunwang.cn/data/?id=100&key=R5RTF4G5F5H&date=".$date;
  ...
  public function get_ymd($caisy,$caism,$caisd){
    ...
    public function category(){
		$id = req::item('id');
		$category = $_COOKIE["category_".$id];
		$moneys1 = $GLOBALS['config']['money']['xmpd'];
        tpl::assign('money1',$moneys1);
        $moneys2 = $GLOBALS['config']['money']['bazijp'];
        tpl::assign('money2',$moneys2);
        $moneys3 = $GLOBALS['config']['money']['hehun'];
        tpl::assign('money3',$moneys3);
        $moneys4 = $GLOBALS['config']['money']['taluotuodan'];
        tpl::assign('money4',$moneys4);
        $moneys5 = $GLOBALS['config']['money']['hmjx'];
        tpl::assign('money5',$moneys5);
        $moneys6 = $GLOBALS['config']['money']['xydd'];
        tpl::assign('money6',$moneys6);
        $moneys7 = $GLOBALS['config']['money']['bzyy'];
        tpl::assign('money7',$moneys7);
        $moneys8 = $GLOBALS['config']['money']['taluobieren'];
        tpl::assign('money8',$moneys8);
        $moneys9 = $GLOBALS['config']['money']['bazi'];
        tpl::assign('money9',$moneys9);
        $moneys10 = $GLOBALS['config']['money']['yinyuancs'];
        tpl::assign('money10',$moneys10);
        $moneys11 = $GLOBALS['config']['money']['taluoaiqing'];
        tpl::assign('money11',$moneys11);
        $moneys12 = $GLOBALS['config']['money']['taluoanlian'];
        tpl::assign('money12',$moneys12);
        $moneys13 = $GLOBALS['config']['money']['taluofenshou'];
        tpl::assign('money13',$moneys13);
        $moneys14 = $GLOBALS['config']['money']['taluofuhe'];
        tpl::assign('money14',$moneys14);
        $moneys15 = $GLOBALS['config']['money']['taluoxinli'];
        tpl::assign('money15',$moneys15);
        $moneys16 = $GLOBALS['config']['money']['ffqm'];
        tpl::assign('money16',$moneys16);
        $moneys17 = $GLOBALS['config']['money']['taluozuihou'];
        tpl::assign('money17',$moneys17);
        $moneys18 = $GLOBALS['config']['money']['aiqingyun'];
        tpl::assign('money18',$moneys18);
        $moneys19 = $GLOBALS['config']['money']['xmfx'];
        tpl::assign('money19',$moneys19);
        $moneys20 = $GLOBALS['config']['money']['bazizh'];
        tpl::assign('money20',$moneys20);
        $moneys21 = $GLOBALS['config']['money']['xingzuo'];
        tpl::assign('money21',$moneys21);
        $moneys22 = $GLOBALS['config']['money']['ziwei'];
        tpl::assign('money22',$moneys22);
        $tpl     = 'ffsm/category.tpl';
        $content = tpl::fetch($tpl);
        exit($content);
    }
    ...
    public function taluoaiqing(){
        ...
        $update = req::item('update',0);//1位强制刷新卡牌
        $pay = req::item('pay',0);
        tpl::assign('sh_title',"2023年塔罗牌爱情运势大揭秘，塔罗占卜你的爱情运势？");
        tpl::assign('sh_desc','单身的你合适能脱单？2023年你的桃花运如何？单身你的会在什么时候找到另外一半？');
...
//生肖运势//
    public function shengxiao(){
		...
		tpl::assign('money',$moneys);
		tpl::assign('sh_title',"2023年十二生肖运势运程,2023生肖运势分析");
        tpl::assign('sh_desc','2023生肖运势专题涵盖2023兔年运势、属鼠、牛、虎、兔、龙、蛇、马、羊、猴、鸡、狗、猪2023年运势及运程，全方位解读十二生肖财运、感情、爱情、事业等运势。');
    ...
    public function bazi(){
		...
		$moneys = $GLOBALS['config']['money']['bazi'];
		tpl::assign('money',$moneys);
		tpl::assign('sh_title',"八字精批、2023流年运程详解、八字测算服务");
            ...
            if($token==base64_encode(md5($oid))){
                ...
                if($row['status']!=1){
					...
					if($row['xydd_img']==""){
						...
						$src_path = "./ffsm/statics/ffsm/xydd/images/light/logo1.png".$xyd_data[$row['data']['lantern_id']][1];
...
//八字综合分析
    public function bazizh(){
		...
		tpl::assign('money',$moneys);
		tpl::assign('sh_title',"四柱八字精批测算、2023流年运程大全-综合运势分析");
...
//PC端测算
    public function pc(){
		...
		tpl::assign('money',$moneys);
		tpl::assign('sh_title',"批八字测姻缘、2023年事业财运详解、塔罗运势占卜");
        tpl::assign('sh_desc','揭开隐藏在你生命中的运势密码，感情事业财运，你和他能不能走到最后。');
        tpl::assign('sh_img',$this->http_type.$_SERVER['HTTP_HOST']."/statics/img/logo_12.png");
		//第四方支付
     	$sys_pay_type=db::queryone("SELECT * FROM `system` WHERE `name` = 'pay_type'");
...
//2023运程
    public function jinnian(){
		...
		tpl::assign('money',$moneys);
		tpl::assign('sh_title',"2023年运势详批-2023流年分析");
...
//八字精批
    public function bazijp(){
		...
		tpl::assign('money',$moneys);
		tpl::assign('sh_title',"四柱八字精批测算、2023流年运程大解析");
        tpl::assign('sh_desc','精准分析→你的人生格局、事业财富、感情健康→');
        tpl::assign('sh_img',$this->http_type.$_SERVER['HTTP_HOST']."/statics/img/bazjp.png");
		//第四方支付
     	$sys_pay_type=db::queryone("SELECT * FROM `system` WHERE `name` = 'pay_type'");
...
//2023爱情
    public function aiqingyun(){
		...
		tpl::assign('money',$moneys);
		tpl::assign('sh_title',"2023年爱情流年运势分析");
    ...
    public function yuanma(){
        ...
        tpl::assign('sh_title',"2023年最新版付费算命源码介绍");

...
class ctl_ffsm_sm_ssys
{
   ...
   public function dl_url($url){
	...
	public function index()
	{
		...
		if(empty($content))
		{
			...
			$seo['title'] = '免费算命的网站_免费算命_生辰八字算命_八字算命_周易算命_算卦在线算命丨远方科技';
			$seo['keywords'] = '免费算命,远方科技,算命网,算命街,周易算命,八字算命,免费算命网站';
			$seo['description'] = '免费算命网站-远方科技提供:免费算命,姓名测试,周易,生辰八字算命,四柱算命预测,算卦,黄道吉日查询,抽签算命,生肖算命,在线排盘和趣味测试等在线免费算命内容！';
			tpl::assign('seo',$seo);
			$tpl     = 'ffsm/index.tpl';
			$content = tpl::fetch($tpl);
  ...
  public function splitName($fullname){ 
	...
	//实时财运
    public function rtcaiyun_example(){
		
		
        $cbid = req::item('cbid');
		$ssysdata = mod_ffsm_ssys::ssysdata_celebrity_ys($cbid);
		...
		//print_r($ssysdata);
		$ssysdata_faq = mod_ffsm_ssys::ssysdata_faq();
		...
		$return['wx_col']=$arrgs_a[$ssysdata['date_jqs']['bz'][0]];
		$return['sx_icon']=$arrzs_a[$ssysdata['date_jqs']['bz'][1]];
		
		$sxarr = array("子"=>'鼠',"丑"=>'牛',"寅"=>'虎',"卯"=>'兔',"辰"=>'龙',"巳"=>'蛇',"午"=>'马',"未"=>'羊',"申"=>'猴',"酉"=>'鸡',"戌"=>'狗',"亥"=>'猪');
		$return['sx']=$sxarr[$ssysdata['date_jqs']['bz'][1]];
		
		
		
	  tpl::assign('return',$return);
	  $date_time=array();
	  $date_time['q3']=date("d",$ssysdata['specify']-3*86400);
	  $date_time['q2']=date("d",$ssysdata['specify']-2*86400);
	  $date_time['q1']=date("d",$ssysdata['specify']-86400);
	  $date_time['td']=date("d",$ssysdata['specify']);
	  $date_time['h1']=date("d",$ssysdata['specify']+86400);
	  $date_time['h2']=date("d",$ssysdata['specify']+2*86400);
	  
	  $date_time['date_q3']=date("Y年m月d日",$ssysdata['specify']-3*86400);
	  $date_time['date_q2']=date("Y年m月d日",$ssysdata['specify']-2*86400);
	  $date_time['date_q1']=date("Y年m月d日",$ssysdata['specify']-86400);
	  $date_time['date_td']=date("Y年m月d日",$ssysdata['specify']);
	  $date_time['date_h1']=date("Y年m月d日",$ssysdata['specify']+86400);
	  $date_time['date_h2']=date("Y年m月d日",$ssysdata['specify']+2*86400);
	  tpl::assign('date_time', $date_time);
	  $tpl     = 'ffsm/ssys/rtcaiyun_rtcaiyun_example.tpl';
    ...
    public function rtcaiyun(){
		...
		if($ssysdata_auth['code']!=200){
      	...
      	$ssysdata_share = mod_ffsm_ssys::ssysdata_share(1);
		...
		$moneys = $GLOBALS['config']['money']['rtcaiyun'];
		...
		//示范例子
		$ssysdata_celebrity = mod_ffsm_ssys::ssysdata_celebrity();
		
		if($ssysdata_celebrity['code']!=200){
			if(empty($ssysdata_auth['message'])){
				$ssysdata_auth['message']="发生未知错误";
			}
			$this->error_bc($ssysdata_auth['message'],'/','返回首页');
		}
		tpl::assign('celebrity',$ssysdata_celebrity);
        ...
        if($username!=''){
            ...
            $datas = array('data'=>urlencode(json_encode($data)),'oid'=>$oid,'createtime'=>time(),'type'=>301,'ip'=>util::get_client_ip(),'des'=>$des,'money'=>$moneys,'pay_uid'=>$pay_uid,'uid'=>$dl_id,'dl_status'=>0,'dl_money'=>$hdjf);
          ...
          $tpl     = 'ffsm/ssys/rtcaiyun_order.tpl';
        }elseif($oid!=''){
            if($token==base64_encode(md5($oid))){
                ...
                if($row['status']!=1){
                    ...
                    $ssysdata = mod_ffsm_ssys::ssysdata("csapi_seek",$userdata['y'],$userdata['m'],$userdata['d'],$userdata['h'],$oid,$row['money']);
				  ...
				  tpl::assign('oid',$row['oid']);
					...
					$this->dl_js($row);
                    $tpl     = 'ffsm/ssys/rtcaiyun_find.tpl';
                }
            }else{
                die('验证失败!');
            }
        }else{
            $tpl     = 'ffsm/ssys/rtcaiyun.tpl';
        }

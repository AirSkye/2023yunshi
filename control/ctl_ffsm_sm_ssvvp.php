  ...
  public function splitName($fullname){ 
    ...
    public function ssvvp(){
		...
		if($ssysdata_auth['code']!=200){
      	...
      	$ssysdata_share = mod_ffsm_ssys::ssysdata_share(10);
        ...
        $lDate2 = req::item('lDate1');
		$moneys = $GLOBALS['config']['money']['ssvvp'];
        ...
        if($username!=''){
            ...
            $des = $username.'和'.$girl_username."匹配";
			 $data = array('username'=>$username,'gender'=>0,'y'=>$year,'m'=>$month,'d'=>$day,'h'=>$hour,'lDate'=>$lDate,'girl_username'=>$girl_username,'gender2'=>0,'y2'=>$girl_year,'m2'=>$girl_month,'d2'=>$girl_day,'h2'=>$girl_hour,'lDate2'=>$lDate2);
            $datas = array('data'=>urlencode(json_encode($data)),'oid'=>$oid,'createtime'=>time(),'type'=>310,'ip'=>util::get_client_ip(),'des'=>$des,'money'=>$moneys,'pay_uid'=>$pay_uid,'uid'=>$dl_id,'dl_status'=>0,'dl_money'=>$hdjf);
			...
			$return = mod_ffsm_ssys::ssysdouble($datas);

			$data['wx_col']=$arrgs_a[$return['user']['bazi'][0]];
			$data['sx_icon']=$arrzs_a[$return['user']['bazi'][1]];
			
			$data['wx_col2']=$arrgs_a[$return['user2']['bazi'][0]];
			$data['sx_icon2']=$arrzs_a[$return['user2']['bazi'][1]];
			
			tpl::assign('names',$data);
          $tpl     = 'ffsm/ssvvp/order.tpl';
        }elseif($oid!=''){
            if($token==base64_encode(md5($oid))){
                ...
                if($row['status']!=1){
					...
					$return = mod_ffsm_ssys::ssysdouble($row);
					...
					$return['wx_col2']=$arrgs_a[$return['user2']['bazi'][0]];
					$return['sx_icon2']=$arrzs_a[$return['user2']['bazi'][1]];
                   
                    $yuefen = mod_ffsm_ssys::yuefen($return['user']['sx']);
					$userdata=json_decode(urldecode($row['data']),true);
                    $ssysdata = mod_ffsm_ssys::ssnvdata("csapi_ssvvp",$userdata['y'],$userdata['m'],$userdata['d'],$userdata['h'],$userdata['y2'],$userdata['m2'],$userdata['d2'],$userdata['h2'],$oid,$row['money']);
				  ...
				  tpl::assign('oid',$row['oid']);
					...
					$this->dl_js($row);
                    $tpl     = 'ffsm/ssvvp/find.tpl';
                }
            }else{
                die('验证失败!');
            }
        }else{
            $tpl     = 'ffsm/ssvvp/index.tpl';
        }

  ...
  public function splitName($fullname){ 
    ...
    public function ssnnp(){
		...
		if($ssysdata_auth['code']!=200){
      	...
      	$ssysdata_share = mod_ffsm_ssys::ssysdata_share(9);
		...
		$moneys = $GLOBALS['config']['money']['ssnnp'];
        ...
        if($username!=''){
            ...
            $des = $username.'和'.$girl_username."匹配";
			 $data = array('username'=>$username,'gender'=>1,'y'=>$year,'m'=>$month,'d'=>$day,'h'=>$hour,'lDate'=>$lDate,'girl_username'=>$girl_username,'gender2'=>1,'y2'=>$girl_year,'m2'=>$girl_month,'d2'=>$girl_day,'h2'=>$girl_hour,'lDate2'=>$lDate2);
            $datas = array('data'=>urlencode(json_encode($data)),'oid'=>$oid,'createtime'=>time(),'type'=>309,'ip'=>util::get_client_ip(),'des'=>$des,'money'=>$moneys,'pay_uid'=>$pay_uid,'uid'=>$dl_id,'dl_status'=>0,'dl_money'=>$hdjf);
          ...
          $tpl     = 'ffsm/ssnnp/order.tpl';
            ...
            if($token==base64_encode(md5($oid))){
                ...
                if($row['status']!=1){
                    ...
                    $ssysdata = mod_ffsm_ssys::ssnvdata("csapi_ssnnp",$userdata['y'],$userdata['m'],$userdata['d'],$userdata['h'],$userdata['y2'],$userdata['m2'],$userdata['d2'],$userdata['h2'],$oid,$row['money']);
				  ...
				  tpl::assign('oid',$row['oid']);
					...
					$this->dl_js($row);
                    $tpl     = 'ffsm/ssnnp/find.tpl';
                }
            }else{
                die('验证失败!');
            }
        }else{
            $tpl     = 'ffsm/ssnnp/index.tpl';
        }

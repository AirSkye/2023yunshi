...
class ctl_peidui
{
	...
	public function hehun(){
		...
		$topic = mod_topic::get_topic('360',$tid);
		...
		if(req::item('user'))
		{
			...
			$tpl     = 'index/peidui/hehun_find.tpl';
			$content = tpl::fetch($tpl);
			exit($content);
		}
		else
		{
			$tpl     = 'index/peidui/hehun_form.tpl';
			$content = tpl::fetch($tpl);
			exit($content);
		}
	}
	...
	public function xingzuoxuexing(){
		...
		$tpl     = 'index/peidui/xingzuoxuexing.tpl';
		$content = tpl::fetch($tpl);
		exit($content);
	}
	...
	public function shengxiaoxuexing(){
		...
		$tpl     = 'index/peidui/shengxiaoxuexing.tpl';
		$content = tpl::fetch($tpl);
		exit($content);
	}
	...
	public function xingzuoshengxiao(){
		...
		$tpl     = 'index/peidui/xingzuoshengxiao.tpl';
		$content = tpl::fetch($tpl);
		exit($content);
	}
	...
	public function shengxiao(){
		...
		if(req::item('sx1'))
		{
			$sx1=req::item('sx1');
			$sx2=req::item('sx2');
			$sql="select * from `peidui_data` where tid='".$tid."' and t1='".$sx1."' and t2='".$sx2."'";
			$sx=db::queryone($sql);	
			tpl::assign('sx1',$sx1);
			tpl::assign('sx2',$sx2);
			tpl::assign('sx',$sx);
		...
		$tpl     = 'index/peidui/shengxiao.tpl';
		$content = tpl::fetch($tpl);
		exit($content);
	}
	...
	public function xuexing(){
		...
		if(req::item('xx1'))
		{
			$xx1=req::item('xx1');
			$xx2=req::item('xx2');
			$sql="select * from `peidui_data` where tid='".$tid."' and t1='".$xx1."' and t2='".$xx2."'";
			$xx=db::queryone($sql);	
			tpl::assign('xx1',$xx1);
			tpl::assign('xx2',$xx2);
			tpl::assign('xx',$xx);

    ...
    */
    public function index()
    {
        ...
        if(empty($content))
        {
			...
			$topic = mod_topic::get_topic_h5('344',$tid);
	...
	public function show(){
		...
		$topic = mod_topic::get_topic_h5('344','',array('date'=>'news_data','id'=>$id));
		tpl::assign('topic',$topic);
		...
		$seo['description'] = $data['shorttxt']==''?util::cn_truncate($data['content'],80):$data['shorttxt'];
		tpl::assign('seo',$seo);
	...
	public function search(){
		...
		if (!db::num_rows()){
				die("<script> alert('没有相关内容！');parent.location.href='http://wp.xingyunyun.cn/baike';</script>");
        }else{
				...
				/********* KDT *************/
				$seo_header['title']       = $q.'的命理知识，关于'.$q.'的百科知识-远方科技';
				$seo_header['keywords']    = $q;
				$seo_header['description'] = '远方科技为你提供【'.$q.'】的命理知识，为您提供最新最全的【'.$q.'】的百科知识【'.$q.'】。百科知识尽在远方科技。';
				tpl::assign('seo',$seo_header);
				/********* KDT end**********/
				
				$tpl  = 'h5/news_search.tpl';

    ...
    */
	...
	public function lists(){
		...
		if(empty($content))
        {
			...
			$mixdata = $this->items->get_attay_hand_data($handtype_arr);
			tpl::assign('m', $mixdata);//<--END 手动数据
			$topic = mod_topic::get_topic('349',$tid);
			tpl::assign('topic',$topic);
	...
	public function search(){
		$q = req::item('q','');
		if($q=='' || $q=='请输入关键字搜索'){
			die("<script> alert('请输入关键字');parent.location.href='http://bazi.xingyunyun.cn/zgjm';</script>");
		}
        $t = (int)req::item('t',0);
		$tid = 349;
		$path = mod_index::this_path($tid);
		...
		*/
		...
		if (!db::num_rows()){
					...
					die("<script> alert('没有相关内容');parent.location.href='http://bazi.xingyunyun.cn/zgjm';</script>");

  ...
  public function splitName($fullname){ 
    ...
    public function sscp(){
		...
		if($ssysdata_auth['code']!=200){
      	...
      	$ssysdata_share = mod_ffsm_ssys::ssysdata_share(13); 
		...
		$moneys = $GLOBALS['config']['money']['sscp'];
        ...
        if($username!=''){
            ...
            $datas = array('data'=>urlencode(json_encode($data)),'oid'=>$oid,'createtime'=>time(),'type'=>313,'ip'=>util::get_client_ip(),'des'=>$des,'money'=>$moneys,'pay_uid'=>$pay_uid,'uid'=>$dl_id,'dl_status'=>0,'dl_money'=>$hdjf);
          ...
          $tpl     = 'ffsm/sscp/order.tpl';
            ...
            if($token==base64_encode(md5($oid))){
                ...
                if($row['status']!=1){
                    ...
                    $ssysdata = mod_ffsm_ssys::ssysdata("csapi_sscp",$userdata['y'],$userdata['m'],$userdata['d'],$userdata['h'],$oid,$row['money']);
				  ...
				  tpl::assign('oid',$row['oid']);
					...
					$this->dl_js($row);
                    $tpl     = 'ffsm/sscp/find.tpl';
                }
            }else{
                die('验证失败!');
            }
        }else{
            $tpl     = 'ffsm/sscp/index.tpl';
        }
